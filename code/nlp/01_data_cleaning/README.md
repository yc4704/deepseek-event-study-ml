...
1. **`01_transcript_cleaning.ipynb`** — Parse raw JSON transcripts from WRDS, 
   match to CIQ company IDs, extract executive statements (Presenter Speech + 
   Answer), save as `Executives_Transcripts_Cleaned.csv`.

2. **`02_filter_earnings_calls.ipynb`** — Filter to keep only earnings calls,
   extract true call dates from headlines, retain only the latest pre-event 
   call per company. Saves as `Executives_Earnings_Latest_PreEvent.csv`.